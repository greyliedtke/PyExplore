...
class Scene:
    ...
    def clear_time_vals(self):
        ...
        for c in range(16):
            for r in range(11,16):
                clear_array.append(mat.xy_to_i(c, r))
    ...
    def loop(self):
        ...
        if self.time_set:
            # setting time mode
            self.time_color = 'Red'
            self.t_correction += encoder_left.delta*3600 # correct hours
            self.t_correction += encoder_right.delta* 60 # correct minutes
        else:
            self.time_color = 'Sky_Blue'
        ...
        cpp = []
        pa = [mat.xy_to_i(6, 12), mat.xy_to_i(6, 14)]  # initiate with time dots
